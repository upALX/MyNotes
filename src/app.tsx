...
export function App() {
  ...
  function whenNoteDeleted(id: string){
    const newArrayWithoutThisIDNote = notes.filter(note => {
      return note.id != id
    })

    setNotes(newArrayWithoutThisIDNote)

    localStorage.setItem('notes', JSON.stringify(newArrayWithoutThisIDNote))
  }
  ...
  return (
    ...
    <div className="mx-auto max-w-6xl my-12 space-y-6 px-5">
      ...
      <form action="w-full">
        <input type="text" placeholder="I want the note..." className="w-full bg-transparent text-3xl font-semibold tracking-tight placeholder:text-slate-500 border-green-500" onChange={handleUserSearch}/>
      </form>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[250px]">
        ...
        {filtredNotes.map(note => {
          return <CardNote key={note.id} new_note_information={note} whenDeleteNote={whenNoteDeleted}/>
        })}
